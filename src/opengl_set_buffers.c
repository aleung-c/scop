...
void opengl_set_buffers(t_scop *sc)
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * (sc->nb_vertices * 4), sc->obj_vertices, GL_STATIC_DRAW);
	...
	glBufferData(GL_ARRAY_BUFFER, ((sc->total_faces * 3) * 2) * sizeof(float), sc->faces_uv, GL_STATIC_DRAW);
	glVertexAttribPointer(4, 2, GL_FLOAT, GL_FALSE, 0, NULL);
	glEnableVertexAttribArray(4);

	// transition flag buffer
	sc->tbo = 0;
	glGenBuffers(1, &(sc->tbo));

	glBindBuffer(GL_ARRAY_BUFFER, sc->tbo);
	glBufferData(GL_ARRAY_BUFFER, (sc->total_faces * 3) * sizeof(GL_INT), sc->transition_points, GL_STATIC_DRAW);
