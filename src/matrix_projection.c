...
{
	perspective_projection_matrix_part1(sc);
	sc->matrix_perspective_projection[2][0] =
		(sc->camera_right + sc->camera_left) /
		(sc->camera_right - sc->camera_left);
	sc->matrix_perspective_projection[2][1] =
		(sc->camera_top + sc->camera_bottom) /
		(sc->camera_top - sc->camera_bottom);
	sc->matrix_perspective_projection[2][2] =
		-((sc->camera_far + sc->camera_near) /
			(sc->camera_far - sc->camera_near));
	sc->matrix_perspective_projection[2][3] = -1.0;
	sc->matrix_perspective_projection[3][0] = 0.0;
	sc->matrix_perspective_projection[3][1] = 0.0;
	sc->matrix_perspective_projection[3][2] =
		-((2 * sc->camera_far * sc->camera_near)
		/ (sc->camera_far - sc->camera_near));
...
void	perspective_projection_matrix_part1(t_scop *sc)
{
	...
	sc->matrix_perspective_projection[1][1] = (2 * sc->camera_near)
		/ (sc->camera_top - sc->camera_bottom);
